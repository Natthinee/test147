...
sad = "วันนี้มาลองทำเเบบประเมินกันไหม พิมพ์คำว่า 'เริ่มทำ' เมื่อต้องการทำเเบบประเมินนะ หรือลองใช้ ฟังก์ชันอื่น พิมพ์คำว่า 'ฟังก์ชันเพิ่มเติม'"
Normal = "วันนี้อากาศสดใส อ๊ะ!! มาลองทำอะไรเพื่อผ่อนคลายกันเถอะ พิมพ์คำว่า 'อ่านนิยาย' หรือ 'ฟังเพลง' เพื่อผ่อนคลายกันเถอะ'"
anss01 = ['0.','1.','2.','3.']
app = Flask(__name__)
...
def godaun(event):
    ...
    if question== "ถึง":
      ...
      ##################################อยู่ในส่วนเดียวกับข้างบนเเละเเต่เผื่อฟังก์ชันก์เพิ่ม################################
       ...
       kk = str(scoreme2(userid,question))
       if kk=="ไม่ปกติ":
          confirm_template_message = TemplateSendMessage(
              alt_text='Confirm template',
              template=ConfirmTemplate(
        ...
        answer = 'ผิดจ้าาาาาา ⊙﹏⊙'+'\n'+'เฉลย เรือน "B" จ้า เพราะถ้าเกิดนาฬิกาเดินเข็มวินาทีจะชนจ้าาา'+'\n'+'ถ้าจะทำต่อพิมพ์คำว่า "ทำต่อ"'+'\n'+'ถ้าอยากใช้ฟังก์ชันใหม่ ให้พิมพ์ "ฟังก์ชันเพิ่มเติม" นะจ๊ะ'
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=answer))
...
#    os.rename(file, file_path)
   ...
   import requests
   req = requests.get('https://adc65b46.ngrok.io/'+dist_name)
